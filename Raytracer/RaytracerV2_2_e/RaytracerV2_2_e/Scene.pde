class Scene {
  ...
  void addGround(){
    ...
    _Triangles.add(new Triangle(new Vektor(300,500,200),new Vektor(600,500,200),new Vektor(550,400,200))); 
}
  ...
  void add_N_BallsOnACircle(float n) {
    float rad=300;
    float t=2*PI/n;
    _Balls = new ArrayList<VisualObject>();
    for(int i=0;i<=n-1;i++){
      _Balls.add(new VisualObject(new Vektor (rad*cos(i*t)+width/2, rad*sin(i*t)+height/2, 500), 100));
    }
  }
