...
void draw(){
  ...
  for(int i=0;i<gBall.size();i++){
    ...
    if(gBall.get(i).kill()==true){  //Es wird geprüft ob sich ein Ball an der Grenze des Programmfensters befindet der eventuell ausgelöscht werden sollte
     ...
     sb.setTime(50);                //Tic-Zahl für das Spruchbuch wird gesetzt
    } 
  }
...
void keyPressed(){
  if(key==' '){
    float radius = 20;
    gBall.add(new Balls(b.ausgabex()+b.ausgabebreite()/2, b.ausgabey() -radius,radius)); 
  }
}

//void mouseReleased(){
  
//  float radius = 20;         //gewünschte Kantenlänge
//  gBall.add(new Balls(b.ausgabex()+b.ausgabebreite()/2, b.ausgabey() -radius,radius)); //Bei jedem Mausklick wird ein neuer Ball in der Fabrik hergestellt und zwar mittig auf dem Balken
//}
