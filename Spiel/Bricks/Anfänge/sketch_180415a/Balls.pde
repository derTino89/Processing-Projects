...
float balkenbreite;
  ...
  //Abprallen des Balles am sich verlängernden Balken:
    ...
    balkenX=b.ausgabex(); //Koordinate Balkenmittelpunkt
    balkenbreite=b.ausgabebreite(); //Ausdehnungsabfrage des Balkens
    if(!(_x<=balkenX-balkenbreite/2-_d/2 || _x>=balkenX+balkenbreite/2+_d/2)){
      if((_y>=height-100-_d-15 && !(_y>height-100))){    //15 ist die halbe Balkenhöhe
        _vy=-_vy;
