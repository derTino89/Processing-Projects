class Box {
  ...
  float width()
  {
    return _vBoxGroesse.getX();
  }
  //jb20180515
  float height()
  {
    return _vBoxGroesse.getY();
  }

  void growX(float delta)
  {
    _vBoxGroesse.addiereX(delta); // jb20180515 Methode aendert den Vektor, da es sie schon gibt, nutze ich sie 
  }

  Vektor2D position()
  {
    return _vBoxOrt.clone();
  }

  void moveHorizontal(float dx)
  {
    setPosition(_vBoxOrt.addiereV(new Vektor2D(dx, 0)));
  }

  //jb20180515 
  void setPosition(Vektor2D here) {
    _vBoxOrt=here.clone();
  }
  void setSize(Vektor2D size) {
    _vBoxGroesse=size.clone();
  }
