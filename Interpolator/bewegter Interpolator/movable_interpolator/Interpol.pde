class Interpol {
  ...
  Interpol(MovableVektor2D in1, MovableVektor2D in2) {
    ...
    speedX = new Vektor2D(7, -5);
    speedY = new Vektor2D(6, 7);
  }
  ...
  void ticloop() {
    ...
    if (_punkta.x()<=0 || _punkta.x()>=width)
      speedX.setX(-speedX.getX());
    if (_punktb.x()<=0 || _punktb.x()>=width)
      speedY.setX(-speedY.getX());
    if (_punkta.y()<=0 || _punkta.y()>=height)
      speedX.setY(-speedX.getY());
    if (_punktb.y()<=0 || _punktb.y()>=height)
      speedY.setY(-speedY.getY());
  ...
  void render() {
    ellipse(_punkta.x(), _punkta.y(), 20, 20);
    ellipse(_punktb.x(), _punktb.y(), 20, 20);
    ellipse(_runner.getX(), _runner.getY(), 10, 10);
  }
  MovableVektor2D ausgabeA() {
    return _punkta.clone();
  }
  MovableVektor2D ausgabeB() {
    return _punktb.clone();
  }
  Vektor2D runner() {
